...
void GLRenderer::setupShaders() {
    // 创建Shader对象
    ourShader = new Shader("4.4.texture.vs", "4.4.texture.fs");
}

void GLRenderer::setupBuffers() {
    // 顶点数据 - 包含位置、颜色和纹理坐标
    float vertices[] = {
        // 位置              // 颜色           // 纹理坐标 (note that we changed them to 'zoom in' on our texture image)
        ...
        -0.5f,  0.5f, 0.0f,   1.0f, 1.0f, 0.0f,   0.45f, 0.55f  // 左上角
    };
