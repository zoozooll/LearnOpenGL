...
// settings
const unsigned int SCR_WIDTH_DEFAULT = 800;
const unsigned int SCR_HEIGHT_DEFAULT = 600;
...
void GLRenderer::OnInit()
{
    ...
    // -------------------------
    m_pShader = new Shader("6.2.cubemaps.vs", "6.2.cubemaps.fs");
    m_pSkyboxShader = new Shader("6.2.skybox.vs", "6.2.skybox.fs");
...
void GLRenderer::OnDraw()
{
    ...
    m_pShader->setVec3("cameraPos", camera.Position);
    // cubes
    glBindVertexArray(cubeVAO);
...
// -Z (back)
// -------------------------------------------------------
unsigned int GLRenderer::loadCubemap(std::vector<std::string> faces)
{
    unsigned int textureID;
    glGenTextures(1, &textureID);
