...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        renderer.setLastFrame(lastFrame);
...
void processInput(GLFWwindow *window)
{
    ...
    {
        renderer.setHDR(!renderer.getHDR());
        hdrKeyPressed = true;
    }
    ...
    {
        if (renderer.getExposure() > 0.0f)
            renderer.setExposure(renderer.getExposure() - 0.001f);
        else
            renderer.setExposure(0.0f);
    }
