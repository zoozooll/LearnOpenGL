...
GLRenderer::GLRenderer() : m_pShader(nullptr), m_quadVAO(0), m_quadVBO(0), m_diffuseMap(0), m_normalMap(0),
...
void GLRenderer::OnInit()
{
    ...
    // -------------------------
    m_pShader = new Shader("4.normal_mapping.vs", "4.normal_mapping.fs");
...
void GLRenderer::OnDraw()
{
    ...
    if(m_pShader)
    {
        ...
        model = glm::rotate(model, glm::radians((float)TimeUtils::GetTime() * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0))); // rotate the quad to show normal mapping from multiple directions
